...
import {
  ...
  getCubeParametersWithAutoGeneratedValues,
} from "./utils/cube";
import { generateDataModel, getDataModel } from "./utils/file";
import {
  generateDataMining,
...
export const initCube = functions.https.onCall<ICallableRequest<IInitCube>>(
  ...
  async (req): Promise<ICallableResponse<ICubeData>> => {
    ...
    const { fileUid, cubeParameters } = req.data.data;

    try {
      const dataModel = await generateDataModel(uid, fileUid);
      const initialData = calculateInitialData(dataModel.rows);

      const updatedCubeParameters = getCubeParametersWithAutoGeneratedValues(
        cubeParameters,
        initialData
      );
      ...
      await admin
        ...
        .set(updatedCubeParameters);

      const dataMining = await generateDataMining(
        uid,
        dataModel.rows,
        updatedCubeParameters
      );
      ...
      const inventoryPerformance = await generateInventoryPerformance(
        uid,
        updatedCubeParameters.categories,
        dataMining,
        scorecard
      );
      ...
      return {
        ...
        data: {
          baseData: dataMining,
          scorecardData: scorecard,
          paramsData: updatedCubeParameters,
          inventoryPerformanceData: inventoryPerformance,
        },
      };
    } catch (e: any) {
...
export const calculateDataModelInventoryPerformance =
  functions.https.onCall<ICallableRequest>(
    ...
    async (req): Promise<ICallableResponse> => {
      ...
      try {
        const { dataModel, fileUid } = await getDataModel(uid);
        const inventoryPerformance = await getInventoryPerformanceData(uid);
